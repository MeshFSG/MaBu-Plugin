<?php
    ...
    function woo_wholesale_page_call() {
        ...
        if (isset($_POST["submit"]) ) {
            ...
            // checks if the discount table exists for the product tag associated with the product
        ...
        if ($_GET["action"] == "delete") {
            ...
            if (isset($_GET['del_all']) ) {
                ...
                $get_results_ = $wpdb->get_row( "SELECT * FROM $table_name WHERE id =" . $getrowid );
    ...
    function show_discount_product_base_and_gloablly($arr, float $price, $boolean = true) {
        ...
        foreach ($arr as $key => $value) {
            ...
            $html .= "<p>" . $qty . "+</p>";
            $html .= '<hr class="hr-sh">';
            $html .= '<p>$' . number_format($after_discount, 2) . "</p>";
            $html .= "</li>";
        }
        $html .= " </ul>";
        $html .= "</div>";
